...
#slide[#my-outline-codemetal(4)]

#slide[

  = #text(1.5em)[Refinements]

  #v(1em)

  #text(1.5em)[Weave #ttpurple[Assertions] into #ttgreen[Types]]
...
#slide[
  ...
  #text(size: 0.9em)[
    ...
    === _Parametric_ Refinement

    ```rust forall<n: int> fn (i32[n]) -> bool[n > 0]```

    #show: later

    === _Declared_ with `@`-syntax

    ```rust fn (i32[@n]) -> bool[n > 0]```

    #show: later

    === _Desugared_ from Rust
...
#slide[
  ...
  #codebox(pad: .50fr, size: 0.55em)[
    #reveal-code(lines: (7, 10), full: false)[
      ```rust
